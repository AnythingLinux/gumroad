      ...
      context "for a tier with apply_price_changes_to_existing_memberships enabled" do
        ...
        context "when the subscription has a pending plan change" do
          ...
          it "deletes existing plan changes for product price changes, but not user-initiated plan changes" do
            for_price_change = create(:subscription_plan_change, subscription: disabled_subscription, for_product_price_change: true, created_at: 2.minutes.ago)
            by_user = create(:subscription_plan_change, subscription: disabled_subscription, tier: enabled_tier, perceived_price_cents: new_price - 1, created_at: 1.minute.ago)
